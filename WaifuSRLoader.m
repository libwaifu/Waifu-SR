...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated WaifuSR loader file *)


Unprotect[WaifuSR`PackageScope`Private`$TopLevelLoad];
WaifuSR`PackageScope`Private`$TopLevelLoad = MemberQ[$ContextPath, "Global`"];


BeginPackage["WaifuSR`"];


ClearAll[WaifuSR];
WaifuSR::usage = "WaifuSR is a manager head for the WaifuSR package";
...
WaifuSR["LoadingParameters"] := $PackageLoadSpecs
$PackageLoadSpecs =
	Merge[
		{
			With[
...
WaifuSR["PackageListing"] := $PackageListing;
$PackageListing = <||>;
WaifuSR["Contexts"] := $PackageContexts;
If[!ListQ@$PackageContexts,
	$PackageContexts =
		{
			"WaifuSR`",
			"WaifuSR`PackageScope`Private`",
			"WaifuSR`PackageScope`Package`"
		}
];
$PackageDeclared =
...
WaifuSR["FEScopedSymbols"] := $PackageFEHiddenSymbols;
$PackageFEHiddenSymbols = {};
WaifuSR["PackageScopedSymbols"] := $PackageScopedSymbols;
...
WaifuSR["FileContexts"] := $PackageFileContexts;
...
WaifuSR["DeclaredPackages"] := $DeclaredPackages;
...
WaifuSR["LoadedPackages"] := $LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ] :=
	With[{s = PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s, WaifuSR], "`"]
	];
...
WaifuSR::nodep = "Couldn't load dependency `` of type ``";
WaifuSR::nodup = "Couldn't update dependency `` of type ``";
...
PackageInstallPackageDependency[dep_String, ops : OptionsPattern[]] :=
	Block[{retcode, site, path, file, tmp},
		...
		If[retcode < 300,
			...
			DeleteFile[tmp];
			file,
			Message[WaifuSR::nodep, dep, "Package"];
			DeleteFile[tmp];
			$Failed
		]
	];
		...
		Monitor[
			MapThread[
				Check[
					...
					Message[WaifuSR::nodep, #2, "Paclet"];
					$Failed
				]&,
				{
					pacs,
			...
			Monitor[
				MapThread[
					Check[
						...
						Message[WaifuSR::nodup, #2, "Paclet"];
						$Failed
					]&,
					{
						StringDelete[deps, "`"],
...
PackageCatchMessage[
	...
	Replace[
		Thread[Map[Hold[MessageName[WaifuSR, #]]&, msg], Hold],
		{
			{} :>
				Check[expr,
...
PackageFailureException[
	...
	(
		Set[MessageName[WaifuSR, tag], body];
		PackageFailureException[
			MessageName[WaifuSR, tag],
			body,
			ops
...
PackageAttachMainAutocomplete[] :=
	PackageAddAutocompletions[WaifuSR,
		Table[
			Replace[{} -> None]@
				Cases[
					DownValues[WaifuSR],
...
If[
	(Clear@WaifuSR`PackageScope`Private`$loadAbort;!#)&@
		WaifuSR`PackageScope`Private`$loadAbort,
	Unprotect[WaifuSR`PackageScope`Private`$LoadCompleted];
	WaifuSR`PackageScope`Private`$LoadCompleted = True;
	WaifuSR`PackageScope`Private`PackagePostProcessContextPathReassign[]
]